...
.pagination {
	display: flex;
	justify-content: center; /* 페이지 번호 가운데 정렬 */
	margin-top: 20px;
	margin-bottom: 20px;
}
...
<body>
	<div class="container">
		...
		<div class="list">
			<form name="frm"
				action="<%=request.getContextPath()%>/board/boardList.aws"
				method="get">
				<select name="search">
			...
			<tbody>
				...
				<tr>
					...
					<td style="text-align: left;"><a
						href="<%=request.getContextPath()%>/board/boardContent.aws?bidx=<%=bv.getBidx()%>"><%=bv.getSubject()%></a></td>
		...
		<div>
			<div class="pagination">
				...
				%>
				<a
					href="<%=request.getContextPath()%>/board/boardList.aws?page=<%=pm.getStartPage() - 1%>&<%=param%>">◀</a>
				<%
				}
				...
				%>
				<a
					href="<%=request.getContextPath()%>/board/boardList.aws?page=<%=pm.getEndPage() + 1%>&<%=param%>">▶</a>
				<%
				}
				%>
			</div>
